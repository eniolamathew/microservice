...
BEGIN
    ...
    CREATE TABLE IF NOT EXISTS brand.brand_country_restriction (
        ...
        CONSTRAINT fk_brand_country_restriction_brand FOREIGN KEY (brand_id)
            ...
            ON DELETE CASCADE
        -- Consider adding a FK constraint to countries table if it exists:
        -- , CONSTRAINT fk_brand_country_restriction_country FOREIGN KEY (country_id) REFERENCES some_schema.countries(id) ON DELETE CASCADE
    );
