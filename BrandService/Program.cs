...
using BrandService.Repo.Decorators;
...
// Remove UpgradeDBFactory, UpgradeDBProxy registrations — migrations not run here
...
// Add app services, caching, controllers, etc.
builder.Services.AddMemoryCache();
builder.Services.AddControllers();
builder.Services.AddOpenApi();

builder.Services.AddScoped<IBrandDataProcessor, BrandDataProcessor>();
builder.Services.AddScoped<IBrandDomain, BrandRepo>();
builder.Services.Decorate<IBrandDomain, CachedBrandRepo>();
builder.Services.AddScoped<IBrandCache, BrandCache>();

builder.Services.AddAutoMapper(typeof(ConfigureMapping));
builder.Services.AddMicroserviceCaching();

var brandServiceUrl = builder.Configuration.GetValue<string>("Services:BrandService");

var isDevelopment = builder.Environment.IsDevelopment();

builder.Services.AddHttpClient<IBrandApi, BrandApi>(client =>
{
    client.BaseAddress = new Uri(brandServiceUrl);
})
...
// No migration code here — just normal service start
